...
def execute(filters=None):
	...
	data = frappe.db.sql('''SELECT
							...
							WHERE 
								`tabJob Card`.docstatus <> 2 AND `tabJob Card`.status = %(status)s \
								AND `tabJob Card`.start_date BETWEEN %(from_date)s AND %(to_date)s
						 ''', {"from_date": filters.from_date, "to_date": filters.to_date, "status": filters.status}, as_dict=1)
						 
	for row in data:
		if row.start_date and row.end_date and isinstance(row.start_date, datetime.date) and isinstance(row.end_date, datetime.date):
			row.update({"days_in_workshop": (row.end_date - row.start_date).days})
		elif row.start_date and not row.end_date and isinstance(row.start_date, datetime.date):
