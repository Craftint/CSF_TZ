...
from erpnext.purchase_and_stock_management.report.stock_ledger.stock_ledger import get_item_group_condition
...
def execute(filters=None):
	...
	for (company, item, warehouse) in sorted(iwb_map):
		...
		report_data = [item, item_map[item]["item_name"],
			...
			item_map[item]["price_list_rate"], qty_dict.opening_qty,
	...
	for bn in tbin_map:
		for d in data:
			if d[0] == bn["item_code"] and d[5] == bn["warehouse"] in d:
...
def get_item_details(items, sle, filters):
	...
	if items:
		for item in frappe.db.sql("""
			select ti.name, ti.item_name, ti.description, ti.item_group, ti.brand,
			ti.stock_uom,tip.price_list_rate
			from (`tabItem` ti)
			LEFT JOIN (`tabItem Price` tip)
			ON  tip.item_code = ti.name
			where ti.name in ({0})
